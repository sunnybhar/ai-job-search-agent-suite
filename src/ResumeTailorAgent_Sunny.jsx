...
const STORAGE_KEYS = {
  ...
  history: "tailor_sunny_history",
  slots: "resume_slots_sunny",
};
...
const PROFILE = {
  name: "SUNNY BHARGAVA",
  headline:
    "Product Manager | Growth & Subscription | SaaS Platforms | 10 Years Scaling Subscription Platforms",
  contact: "+1 (551) 998-5759 | sb299@fordham.edu | linkedin.com/in/bhargavasunny",
  font: "Calibri",
  underlineSections: false, // Sunny's template: plain bold caps headers
};

const CANDIDATE_CONTEXT = `International MBA candidate at Fordham University, Gabelli School of Business, Class of 2027. ~10 years across product management (Livguard — scaled subscription platform 200→11,000 users), industrial B2B operations and program delivery (Tata Hitachi, Gainwell — mining-site operations, IoT deployment, inventory management), and operations management (OYO). B.Tech Mechanical Engineering, IIT-ISM Dhanbad. Builds AI-powered workflow tools (Python, React, Anthropic API).
TARGET ROLE ARCHETYPES — infer which one this JD is from its title and responsibilities, and let that drive emphasis:
- Product Management → lead with roadmaps, PRDs, user growth, A/B testing, cross-functional delivery
- Program/Project Management → lead with governance, milestones, stakeholder management, delivery at scale
- Operations/Strategy → lead with process improvement, cost reduction, forecasting, efficiency metrics
- Consulting → lead with structured problem solving, quantified impact, executive communication
No US brand-name employer pedigree — position international experience as scale and complexity, never apologize for it. Be honest and direct.`;
...
- ALL CAPS section headers only: EDUCATION, SKILLS, EXPERIENCE, TECHNICAL PROJECTS, AWARDS & CERTIFICATION (use the sections that exist in the original)
...
// ─────────────────────────────────────────────────────────────────
// DOCX EXPORT — mirrors Sunny's real resume layout
// (Calibri, centered 16pt name, headline, bold CAPS sections, 9pt bullets)
// ─────────────────────────────────────────────────────────────────
...
export default function ResumeTailorAgent() {
  ...
  async function handleDocx() {
    ...
    await downloadDocx(result.rewritten, PROFILE, `Sunny_Bhargava_Resume_${company}`);
  }
